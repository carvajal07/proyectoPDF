...
_GLOBAL_REGISTRY = {
    ...
    "fonts_loaded": False,
    "images_loaded": False,
    "grouping_elements": False,
    "parastyles_loaded": False,
    "borderstyles_loaded": False
}
...
class XMLToPDFConverter:
    ...
    def process_image_objects_v3(self, element_object, c, parent_offset=(0,0)):
        ...
        image_id = imageObject_object.find('ImageId').text
        img = self.get_imagen_from_cache(image_id)
        #img = self.images_cache.get(image_id)
        
        if pos is not None and size is not None and img is not None:
            x = self.convert_units(pos.get('X')) + offset_x
            y = self.convert_units(pos.get('Y')) + offset_y
            width = self.convert_units(size.get('X'))
            height = self.convert_units(size.get('Y'))
    ...
    def get_imagen_from_cache(self, image_id):
        """
        Devuelve una instancia de ImageReader a partir del diccionario self.image_cache
        Soporta imágenes normales y PDF (solo primera página)
        """
        if image_id not in self.images_cache:
            raise ValueError(f"Imagen no encontrada en cache: {image_id}")

        image_bytes = self.images_cache[image_id]
        buffer = BytesIO(image_bytes)

        # Intentar detectar si es PDF o imagen normal
        try:
            if image_bytes[:4] == b'%PDF':
                # Imagen es un PDF → renderizar primera página
                ...
                # Imagen es PNG, JPG, etc.
                return ImageReader(buffer)
        except Exception as e:
            raise ValueError(f"No se pudo procesar imagen {image_id}: {e}")

    def register_images(self):
        images = self.Image_config
        #images_dict = self.images_cache
        #image_dict = getattr(self,"images_cache")
        for id, image in images.items():
            image_id = image.find('Id').text
            image_type = image.find('ImageType').text
            ...
            if isinstance(path, str):
                if path and os.path.exists(path):
                    with open(path, "rb") as f:
                        # Guarda en el cache (sea imagen o None si no existe)
                        self.images_cache[image_id] = f.read()
                    ...
                    sys.exit(1)
                # Guarda en el cache (sea imagen o None si no existe)
                #self.images_cache[image_id] = img
            else:
                print(f"Error: El path de la imagen no es una cadena válida: {path}")
                sys.exit(1)
